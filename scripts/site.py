#!/usr/bin/env python3
"""
site.py — повна версія випуску як сторінка на GitHub Pages.
...
def archive_page(items):
    rows = "".join(
        f'<a href="{iso}.html">{esc(title)}<span>{iso}</span></a>'
        for iso, title in items)
    return f"""<!doctype html>
